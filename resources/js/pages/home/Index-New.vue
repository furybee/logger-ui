    ...
    <div class="flex flex-col h-screen overflow-hidden">
        ...
        <div class="bg-base-300 text-center p-1 text-xs">
            <div v-if="pagination" >
                <button
                    class="btn btn-xs"
                    :disabled="!pagination.prevCursorUrl"
                    @click="refreshLogs(pagination.prevCursorUrl)"
                >
                    Previous
                </button>
                ...
                <button
                    class="btn btn-xs"
                    :disabled="!pagination.nextCursorUrl"
                    @click="refreshLogs(pagination.nextCursorUrl)"
                >
                    Next
                </button>
            </div>
...
import {LogFiltersType, PaginatorType, RefreshLogParamsType} from "../../types";
...
const pagination = ref<PaginatorType>(null);
...
const loadMore = async () => {
    ...
    await refreshLogs({
        url: pagination.value.next_page_url,
    });
...
const refreshLogs = async (params: RefreshLogParamsType = { url: "/logger-ui/logs", forceScrollToBottom: false, customFilters: {} }) => {
    ...
    // refreshLogsUrl = pagination.value.prev_page_url ?? '/logger-ui/logs';
